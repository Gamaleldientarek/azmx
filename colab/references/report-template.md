...
1. **The Counted Field** — the slide *is* the dataset: every insight rendered as a real **20px** module (`180 ÷ 24 = 7.5` — a 24px cell can never land on a column edge; see `layout-archetypes.md` §4.1) (orange = critical, Electric = major, Electric@**40** = minor — **corrected**: Electric@24 composites to **1.93:1** on Deep Jade, below the 3:1 non-text floor, so 24% renders counted data invisible. 40% is the floor on Deep Jade, 50% on Pine (`layout-archetypes.md` §4.4)), the hero numeral carved out of the field's void; a thin hairline ledger of secondary stats on the left. Nothing on the slide is a container.
